...
public:
    ...
    Hospital(const Hospital& initial_value) = delete;
    Hospital& operator=(const Hospital& assignable_value) = delete;
...
private:
    ...
    // Record all of the patients in the past until now
    std::map<std::string, Person*> all_patients_;
    // Record all the care periods
    std::vector<std::pair<std::string, CarePeriod*>> care_periods_;

    // Print a given patient list
    void print_a_patient_list(const std::map<std::string, Person*>& list);

    // Print a given medicines list
    void print_medicine_list(std::map<std::string, std::set<std::string>> medicines);

    // Notice the existence of a person
    bool already_exists(const std::string& id, const std::map<std::string, Person*>& list);
    // Notice if someone can not be found
    bool cant_find(const std::string& id, const std::map<std::string, Person*>& list);
